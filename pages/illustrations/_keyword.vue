...
export default Vue.extend({
  ...
  async fetch({ store, params }) {
    ...
    try {
      const res = await store.dispatch("getSearchResults", { asset: 'illustrations' });
  ...
  methods: {
    ...
    async getSearchSuggestion(val: string) {
      ...
      try {
        ...
        await this.getSearchResults({ asset: 'illustrations' });
      } catch (error) {
        ...
        entries.forEach(async (entry) => {
          if (entry.isIntersecting) {
            ...
            try {
              ...
              this.updateAnOptionProperty({ key: "query", value: val });
              await this.getSearchResults({ loadMoreData: true, asset: 'illustrations' });
            } catch (error) {
